...
<?php if (!empty($kehoach)): ?>
    <script>
        ...
        // Lọc dây chuyền theo phân xưởng của xưởng trưởng
        const maPhanXuong = <?php echo json_encode($_SESSION['user']['BoPhan'] ?? ''); ?>;
        let filteredDayChuyenList = [];
        if (Array.isArray(dayChuyenList = <?php echo json_encode($dayChuyenList ?? []); ?>)) {
