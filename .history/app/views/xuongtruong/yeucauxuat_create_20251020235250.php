<?php
// Form tạo phiếu yêu cầu xuất nguyên liệu cho xưởng trưởng
?>
<div class="container mt-3">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0">Tạo phiếu yêu cầu xuất nguyên liệu</h4>
    <div>
      <a class="btn btn-outline-secondary btn-sm" href="<?php echo BASE_URL; ?>xuongtruong/dashboard"></a>Về Dashboard</a>
      <a class="btn btn-outline-primary btn-sm" href="<?php echo BASE_URL; ?>xuongtruong/yeucauxuat_list">Danh sách phiếu</a>
    </div>
  </div>

  <form id="frmYeuCauXuat" method="post" action="<?php echo BASE_URL; ?>yeucauxuat/save" class="row g-3">
    <input type="hidden" name="_csrf" value="<?php echo md5(session_id()); ?>">
    <input type="hidden" name="action" id="action" value="send">

    <div class="col-md-6">
      <label class="form-label">Chọn kế hoạch sản xuất</label>
      <select name="ma_kehoach" class="form-select" required>
    ...
    <div class="col-12 d-flex gap-2 justify-content-end">
      <button type="button" class="btn btn-outline-secondary" onclick="document.getElementById('frmYeuCauXuat').reset()">Làm mới</button>
      <button type="submit" class="btn btn-primary" onclick="document.getElementById('action').value='send'">Gửi yêu cầu</button>
      <button type="submit" class="btn btn-warning" onclick="document.getElementById('action').value='draft'">Lưu nháp</button>
    </div>
  </form>
</div>
