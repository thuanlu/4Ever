...
class KeHoachSanXuatController extends BaseController {
    ...
    private function loadCreateView($extra_data = []) {
        ...
        $data['latestMaDonHang'] = !empty($available_donhangs) ? $available_donhangs[0]['MaDonHang'] : '';
    ...
    public function edit($maKeHoach) {
        ...
        if ($kehoach['TrangThai'] !== 'Chờ duyệt' && $kehoach['TrangThai'] !== 'Từ chối') {
            $_SESSION['error'] = 'Không thể sửa kế hoạch đã được duyệt hoặc hoàn thành.';
