...
class LapKeHoachCapXuongController extends BaseController {
    public function index() {
        ...
        if (isset($_GET['kehoach'])) {
            ...
            // Bổ sung lấy mã sản phẩm, tên sản phẩm, số lượng sản phẩm từ ChiTietKeHoach
            ...
            if (!empty($chiTietList)) {
                $selectedKeHoach['MaSanPham'] = $chiTietList[0]['MaSanPham'] ?? '';
                $selectedKeHoach['TenSanPham'] = $chiTietList[0]['TenSanPham'] ?? '';
                $selectedKeHoach['SoLuongSanPham'] = $chiTietList[0]['SoLuong'] ?? '';
            } else {
                $selectedKeHoach['MaSanPham'] = '';
                $selectedKeHoach['TenSanPham'] = '';
                $selectedKeHoach['SoLuongSanPham'] = '';
            }
        }
    ...
    public function store() {
        ...
        try {
            ...
            // Sinh mã kế hoạch cấp xưởng tự động
            $prefix = 'KHCX';
            $datePart = date('ymdHis');
            $randPart = rand(10,99);
            $maKHCapXuong = $prefix . $datePart . $randPart;
