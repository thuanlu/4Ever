        ...
        <h5 class="mt-2 text-primary fw-bold">1. Thông tin chung</h5>
         ...
         <div class="col-md-<?php echo ($is_editing || $is_viewing) ? 8 : 12; ?> mb-3">
             ...
             <select name="MaDonHang" id="MaDonHangSelect" class="form-select" required <?php echo ($is_viewing || $is_editing) ? 'disabled' : ''; ?>>
                 ...
                 <?php $selectedMaDonHang = get_value('MaDonHang', $kehoach); 
                 if (!$is_editing && !$is_viewing && empty($selectedMaDonHang)) {
                        $selectedMaDonHang = $latestMaDonHang ?? '';
                    }
...
<div class="col-md-3 mb-3">
    ...
    <td>
        <input type="text" name="products[<?php echo $index; ?>][materials][<?php echo $i; ?>][MaNguyenLieu]" class="form-control form-control-sm" value="<?php echo $nl['MaNguyenLieu']; ?>" readonly>
    </td>
    
    <td>
        <input type="text" name="products[<?php echo $index; ?>][materials][<?php echo $i; ?>][TenNguyenLieu]" class="form-control form-control-sm" value="<?php echo $nl['TenNguyenLieu']; ?>" readonly>
    </td>
    
    <td>
        <input type="text" class="form-control form-control-sm" value="<?php echo $donViTinh; ?>" readonly>
    </td>

    <td>
        <input type="number" step="any" name="products[<?php echo $index; ?>][materials][<?php echo $i; ?>][DinhMucBOM]" class="form-control form-control-sm bom-input" value="<?php echo number_format($dinhMuc, 2, '.', ''); ?>" readonly>
    </td>

    <td>
        <input type="number" step="any" class="form-control form-control-sm required-qty-input" value="<?php echo $soLuongCan; ?>" readonly>
    </td>

    <td>
        <input type="number" step="any" name="products[<?php echo $index; ?>][materials][<?php echo $i; ?>][TonKho]" class="form-control form-control-sm stock-input" value="<?php echo $tonKho; ?>" readonly>
    </td>

    <td>
        <input type="number" step="any" name="products[<?php echo $index; ?>][materials][<?php echo $i; ?>][CanBoSung]" class="form-control form-control-sm needed-input" value="<?php echo $canBoSung; ?>" readonly>
    </td>
</tr>
                            <?php endforeach; ?>
                        </tbody>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function buildProductHtml(products, bom_data) {
             ...
             products.forEach((product, index) => {
                 ...
                 if (xuongList && xuongList.length > 0) {
                     // 1. Chuẩn hóa tên sản phẩm về chữ thường để so sánh
                     const lowerTenSP = tenSP.toLowerCase();
                     
                     // Cờ để chỉ chọn 1 xưởng duy nhất mặc định
                     let hasSelected = false; 

                     xuongList.forEach(xuong => {
                         // Lấy số ID từ mã xưởng (Ví dụ: "PX05" -> 5, "PX10" -> 10)
                         // replace(/\D/g, '') sẽ loại bỏ chữ, chỉ giữ số
                         const xuongIdNum = parseInt(xuong.MaPhanXuong.replace(/\D/g, ''), 10);
                         
                         let isSelected = false;

                         // Logic: Nếu chưa chọn xưởng nào cho dòng này thì mới xét tiếp
                         if (!hasSelected) {
                             // Nếu tên có chữ "giày" -> Chọn xưởng 1-5
                             if (lowerTenSP.includes('giày') && xuongIdNum >= 1 && xuongIdNum <= 5) {
                                 isSelected = true;
                                 hasSelected = true;
                             }
                             // Nếu tên có chữ "dép" -> Chọn xưởng 6-10
                             else if (lowerTenSP.includes('dép') && xuongIdNum >= 6 && xuongIdNum <= 10) {
                                 isSelected = true;
                                 hasSelected = true;
                             }
                         }
                         ...
                         productHtml += `<option value="${xuong.MaPhanXuong}" ${selectedAttr}>${xuong.MaPhanXuong} - ${xuong.TenPhanXuong}</option>`;
                     });
                 }
                 // --- ===== KẾT THÚC SỬA LỖI ===== ---
        ...
        if (maDonHangSelect && maDonHangSelect.value && !<?php echo $is_viewing ? 'true' : 'false'; ?> && !<?php echo $is_editing ? 'true' : 'false'; ?>) {
            // Tạo sự kiện change
            const event = new Event('change');
            // Dispatch sự kiện
            maDonHangSelect.dispatchEvent(event);
        }
