    ...
    <div class="pt-4 mt-2">
        <div class="card mb-4">
            ...
            <form method="POST" action="<?= BASE_URL ?>xuongtruong/lapkehoachcapxuong/store" id="form-lapkehoach" class="p-3">
                ...
                <div class="mb-3">
                    <div id="ca-list" class="row g-3 align-items-end">
                        <!-- Ca làm việc động -->
                    </div>
...
<pre>
<?php
echo 'Session MaXuongTruong: ';
print_r($_SESSION['user']['MaNV'] ?? '');
echo "\nPhanXuongList:\n";
print_r($phanXuongList);
echo "\nDayChuyenList:\n";
print_r($dayChuyenList);
?>
</pre>
<?php if (!empty($kehoach)): ?>
    <script>
        ...
        let maPhanXuong = '';
        if (Array.isArray(phanXuongList)) {
            const found = phanXuongList.find(px => px.MaXuongTruong === maXuongTruong);
            if (found) maPhanXuong = found.MaPhanXuong;
        }
        const dayChuyenList = <?php echo json_encode($dayChuyenList ?? []); ?>;
        let filteredDayChuyenList = [];
        if (Array.isArray(dayChuyenList)) {
        ...
        function addCa() {
            caIndex++;
            const dcOptions = filteredDayChuyenList.map(dc => `<option value='${dc.MaDayChuyen}'>${dc.TenDayChuyen}</option>`).join('');
            const caHtml = `<div class="mb-2" id="ca-item-${caIndex}">
            <div class="row g-2 align-items-center">
                <div class="col-md-4">
                    <label class="form-label fw-bold mb-1">Dây chuyền</label>
                    <select name="ca[${caIndex}][day_chuyen]" class="form-control" onchange="updateToTruong(${caIndex})" id="daychuyen-${caIndex}">
                        ${dcOptions}
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold mb-1">Sản lượng mục tiêu</label>
                ...
                <div class="col-md-3">
                    <label class="form-label fw-bold mb-1">Tổ trưởng phụ trách</label>
                ...
                <div class="col-md-1 text-end d-flex align-items-end" style="height:100%">
                    <button type="button" class="btn btn-danger mt-4" onclick="removeCa(${caIndex})"><i class="fa fa-trash"></i></button>
                </div>
            </div>
        </div>`;
