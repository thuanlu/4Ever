                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <?php
                                ...
                                $soThuTu = (isset($kehoachCapXuongs) && is_array($kehoachCapXuongs)) ? count($kehoachCapXuongs) + 1 : 1;
                                $autoMaKeHoachXuong = 'KCX-' . date('Ymd') . '-' . $soThuTu;
                            ?>
...
<?php if (!empty($kehoach)): ?>
    <script>
        ...
        const dayChuyenList = <?php echo json_encode($dayChuyenList ?? []); ?>;
            // Danh sách dây chuyền đã được chọn và chưa hết hạn
            const lockedDayChuyen = [];
            <?php
            $today = date('Y-m-d');
            if (!empty($kehoachCapXuongs)) {
                foreach ($kehoachCapXuongs as $khcx) {
                    if (!empty($khcx['MaDayChuyen']) && !empty($khcx['NgayKetThuc'])) {
                        if ($today < $khcx['NgayKetThuc']) {
                            echo "lockedDayChuyen.push('{$khcx['MaDayChuyen']}');\n";
                        }
                    }
                }
            }
        ...
        function addCa() {
            ...
            if (filteredDayChuyenList.length > 0) {
                    dcOptions = filteredDayChuyenList
                        .filter(dc => !usedDayChuyen.includes(dc.MaDayChuyen) && !lockedDayChuyen.includes(dc.MaDayChuyen))
                ...
                let currentValue = sel.value;
                    let options = filteredDayChuyenList
                        .filter(dc => (!selected.includes(dc.MaDayChuyen) || dc.MaDayChuyen === currentValue)
                            && !lockedDayChuyen.includes(dc.MaDayChuyen))
